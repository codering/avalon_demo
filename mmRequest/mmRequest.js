...
define(['avalon', 'mmPromise/mmPromise'], function (avalon) {
    ...
    originAnchor.href = location.href;    //告诉WEB服务器自己接受什么介质类型，*/* 表示任何类型，type/* 表示该类型下的所有子类型，type/sub-type。
    ...
    function ajaxExtend(opts) {
        ...
        if (typeof opts.crossDomain !== 'boolean') {
            //判定是否跨域
            var urlAnchor = document.createElement('a');    // Support: IE6-11+
                                                            // IE throws exception if url is malformed, e.g. http://example.com:80x/
            // Support: IE6-11+
            // IE throws exception if url is malformed, e.g. http://example.com:80x/
            try {
                urlAnchor.href = opts.url;    // in IE7-, get the absolute path
    ...
    var XHRMethods = {
        ...
        dispatch: function (status, nativeStatusText) {
            var statusText = nativeStatusText;    // 只能执行一次，防止重复执行
            // 只能执行一次，防止重复执行
            if (!this.transport) {
                //2:已执行回调
                return;
             ...
             * global event handler
             */
            var that = this;    // 到这要么成功，调用success, 要么失败，调用 error, 最终都会调用 complete
    ...
    avalon.serialize = function (form) {
        var json = {};    // 不直接转换form.elements，防止以下情况：   <form > <input name="elements"/><input name="test"/></form>
    ...
    var transports = avalon.ajaxTransports = {
        xhr: {
            ...
            request: function () {
                ...
                //必须要支持 FormData 和 file.fileList 的浏览器 才能用 xhr 发送
                //标准规定的 multipart/form-data 发送必须用 utf-8 格式， 记得 ie 会受到 document.charset 的影响
                transport.send(opts.hasContent && (this.formdata || this.querystring) || null);    //在同步模式中,IE6,7可能会直接从缓存中读取数据而不会发出请求,因此我们需要手动发出请求
                ...
                try {
                    ...
                    if (forceAbort || completed) {
                        ...
                        if (forceAbort) {
                            ...
                            var status = transport.status;    //设置responseText
                            //设置responseText
                            var text = transport.responseText;
                            this.responseText = typeof text === 'string' ? text : void 0;    //设置responseXML
                            //设置responseXML
                            try {
        ...
        avalon.fixAjax = function () {
            ...
            function addDataToForm(form, data) {
                ...
                for (d in data) {
                    ...
                    vs = isArray ? data[d] : [data[d]];    // 数组和原生一样对待，创建多个同名输入域
                    // 数组和原生一样对待，创建多个同名输入域
            ...
            avalon.ajaxTransports.upload = {
                request: function () {
                    ...
                    var form = opts.form;
                    var iframe = this.transport = createIframe(ID);    //form.enctype的值
                    ...
                    var fields = opts.data ? addDataToForm(form, opts.data) : [];    //必须指定method与enctype，要不在FF报错
                ...
                respond: function (event) {
                    var node = this.transport, child;    // 防止重复调用,成功后 abort
                    // 防止重复调用,成功后 abort
                    if (!node) {
                        return;
                    }
                    if (event && event.type === 'load') {
                        ...
                        if (doc.body) {
                            ...
                            this.responseText = doc.body.innerHTML;    //当MIME为'application/javascript' 'text/javascript",浏览器会把内容放到一个PRE标签中
