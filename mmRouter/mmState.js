define(['mmPromise/mmPromise', 'mmRouter/mmRouter'], function () {
    ...
    var mmState = window.mmState = {
        ...
        pushOne: function (chain, params, callback, _local, toLocals) {
            var cur = chain.shift(), me = this;    // 退出
            // 退出
            if (!cur) {
                return callback();
            }
            cur.syncParams(params);    // 阻止进入该状态
            // 阻止进入该状态
            ...
            cur.done = function (success) {
                ...
                resolved.$then(function (res) {
                    var a = resolved;    // sync params to oldParams
                    // sync params to oldParams
                    avalon.mix(true, cur.oldParams, cur.params);    // 继续状态链
                    // 继续状态链
        ...
        transitionTo: function (fromState, toState, toParams, options) {
            var toParams = toParams || toState.params, fromAbort;    // state machine on transition
            ...
            var exitChain = fromChain.slice(i),
                ...
                // 需要进入的chain
                commonLocal = _local;    // 建立toLocals，用来计算哪些view会被替换
            ...
            if (!reload && fromState === toState) {
                ...
                if (!changeType) {
                    ...
                    if (toState == this.activeState && fromAbort)
                        return done();    // 重复点击直接return
                    // 重复点击直接return
                    return;
                }
            }
            mmState.query = avalon.mix({}, toParams.query);    // onBeforeUnload check
    ...
    function broadCastBeforeUnload(exitChain, enterChain, fromState, toState) {
        ...
        for (var i in lastLocal) {
            var local = lastLocal[i];    // 所有被更新的view
            // 所有被更新的view
    ...
    avalon.bindingHandlers.view = function (data, vmodels) {
        ...
        function update(firsttime, currentState, changeType) {
            ...
            if (firsttime && !_local || currentLocal === _local)
                return;
            currentLocal = _local;
            _currentState = _local && _local.state;    // 缓存，如果加载dom上，则是全局配置，针对template还可以开一个单独配置
            ...
            avalon.clearHTML(element);    // oldElement = element
            // oldElement = element
            element.removeAttribute('ms-view');
            element.setAttribute('ui-view', data.value);    // 本次更新的dom需要用缓存
            // 本次更新的dom需要用缓存
            if (cacheTpl) {
                // 已缓存
                ...
                element.appendChild(fragment);    // 更新现在使用的cache名字
                // 更新现在使用的cache名字
            ...
            // merge上下文vmodels + controller指定的vmodels
            avalon.scan(element, (_local && _local.vmodels || []).concat(vmodels || []));    // 触发视图绑定的controller的事件
    ...
    avalon.state = function (stateName, opts) {
        ...
        avalon.router.get(state.url, function (params, _local) {
            ...
            avalon.each(state.views, function (name, view) {
                ...
                promises.push(promise);    // template不能cache
                // template不能cache
