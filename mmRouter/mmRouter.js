    ...
    Router.prototype = {
        ...
        _pathToRegExp: function (pattern, opts) {
            ...
            while (m = placeholder.exec(pattern)) {
                ...
                keys.push(key);
                compiled += quoteRegExp(segment, regexp, false);    //  segments.push(segment)
                //  segments.push(segment)
                last = placeholder.lastIndex;
            }
            segment = pattern.substring(last);
            compiled += quoteRegExp(segment) + (opts.strict ? opts.last : '/?') + '$';
            var sensitive = typeof opts.caseInsensitive === 'boolean' ? opts.caseInsensitive : true;    //  segments.push(segment);
            //  segments.push(segment);
        ...
        navigate: function (hash, options) {
            ...
            if (!avalon.state || options.silent)
                avalon.history && avalon.history.updateLocation(hash, avalon.mix({}, options, { silent: true }));    // 只是写历史而已
            // 只是写历史而已
            if (!options.silent) {
