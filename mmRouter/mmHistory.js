define(['avalon'], function (avalon) {
    ...
    History.started = false;    //取得当前IE的真实运行环境
    //取得当前IE的真实运行环境
    ...
    History.prototype = {
        ...
        start: function (options) {
            ...
            this.options = avalon.mix({}, History.defaults, options);    //IE6不支持maxHeight, IE7支持XMLHttpRequest, IE8支持window.Element，querySelector, 
            ...
            //IE9支持window.Node, window.HTMLElement, IE10不支持条件注释
            //确保html5Mode属性存在,并且是一个布尔
            this.html5Mode = !!this.options.html5Mode;    //监听模式
            //监听模式
            ...
            this.prefix = '#' + this.options.hashPrefix + '/';    //确认前后都存在斜线， 如"aaa/ --> /aaa/" , "/aaa --> /aaa/", "aaa --> /aaa/", "/ --> /"
            ...
            function checkUrl(e) {
                ...
                if (iframe) {
                    //IE67
                    var iframeHash = that.getHash(iframe);    //与当前页面hash不等于之前的页面hash，这主要是用户通过点击链接引发的
                    ...
                    } else //如果是后退按钮触发hash不一致
                    if (iframeHash !== that.fragment) {
        ...
        updateLocation: function (hash, options, urlHash) {
            ...
            if (this.monitorMode === 'popstate') {
                ...
                var path = this.rootpath + hash + (urlHash || '');    // html5 model包含query
                // html5 model包含query
    ...
    avalon.history = new History();    //https://github.com/asual/jquery-address/blob/master/src/jquery.address.js
    ...
    avalon.bind(document, 'click', function (event) {
        var defaultPrevented = 'defaultPrevented' in event ? event['defaultPrevented'] : event.returnValue === false, routeElementJudger = avalon.history.options.routeElementJudger;
